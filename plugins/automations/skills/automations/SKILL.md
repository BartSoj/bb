...
Use `script` when the output is fully determined by code: watchdogs, threshold alerts, health checks, heartbeats, and API pollers with a fixed output shape. Scripts run on the bb server, with cwd set to the project's default source directory when that directory exists on the server host, and otherwise to the plugin data directory's `scripts/` area. Script automations do not have an environment field and do not accept environment flags.
